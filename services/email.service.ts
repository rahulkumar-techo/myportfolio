...
const gmailInstance ={
    ...
    secure:true,
    auth:{
        user:"mrrhl02@gmail.com",
        pass:process.env.GMAIL_LESSSECURE_PASSWORD
    }
}
...
export async function sendProjectMail({ user, project }: SendMailProps) {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "https://rahulkumardev.vercel.app";
  const html = generateProjectEmailTemplate({
    username: user.name,
    email: user.email,
    userAvatar: user.image || "https://cdn-icons-png.flaticon.com/512/149/149071.png",
  ...
  await transporter.sendMail({
    from: `Rahul Portfolio <${process.env.GMAIL_EMAIL}>`,
    to: user.email,
